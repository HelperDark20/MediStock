...
function populateNacimientoSelects(){
  const selDia = document.getElementById('u-nac-dia');
  const selMes = document.getElementById('u-nac-mes');
  const selAño = document.getElementById('u-nac-año');
  if(!selDia || !selMes || !selAño) return;
  ...
  selAño.innerHTML = '<option value="">Año</option>' +
    Array.from({length:80}, (_,i) =>
      `<option value="${añoActual - i}">${añoActual - i}</option>`
    ).join('');
}
...
function renderUsuarios(){
  // Poblar selects de fecha cada vez que se renderiza la vista
  populateNacimientoSelects();
